...
//    X% del CAPITAL que quede pendiente en ese momento.
...
function calcularDeuda(importe, tasaPct, fechaInicio, pagos, hasta) {
  ...
  let n = 1;
  let corte = sumarMeses(inicio, 1);
  const movs = [];

  function devengarHasta(f) {
    if (tasa <= 0) return;
    let guard = 0;
    while (capPend > 0 && corte.getTime() <= f.getTime() && guard < 1200) {
      const i = pesos(capPend * tasa);
      if (i > 0) {
        intPend += i; intGen += i;
        movs.push({ tipo: 'interes', fecha: corte.toISOString(), monto: i, mes: n, sobre: capPend });
      }
      n++; guard++;
